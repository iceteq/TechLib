import { Layers, Minus, Plus, Printer, Trash2, X } from 'lucide-react';
import type {
  CartItem,
  Label,
  NoteType,
  NoteWithUrls,
  StockLocation,
} from '../../lib/types';
import { Barcode } from '../barcodes/Barcode';
import { noteTypeById, noteTypeIcon } from '../../lib/noteTypes';
import { categoryLabel } from '../../lib/searchNotes';
import { resolveGuidelineLines } from '../../lib/guidelineLines';
import { DISPOSITIONS } from '../../lib/types';
import { NoteCard } from './NoteCard';
import styles from './CollectionView.module.css';

export type CollectionRow = {
  item: CartItem;
  note: NoteWithUrls | null;
};

interface CollectionViewProps {
  rows: CollectionRow[];
  labels: Label[];
  noteTypes: NoteType[];
  stockLocations: StockLocation[];
  unitCount: number;
  showBarcodes: boolean;
  showPhotos: boolean;
  showDescription: boolean;
  showSpecialCase: boolean;
  showLabels: boolean;
  showAge: boolean;
  showTypeChip: boolean;
  onOpenNote: (noteId: string) => void;
  onChangeQuantity: (noteId: string, quantity: number) => void;
  onRemove: (noteId: string) => void;
  onClear: () => void;
  /** One-tap path back to the note wall when the collection is empty. */
  onBrowseNotes?: () => void;
}

export function CollectionView({
  rows,
  labels,
  noteTypes,
  stockLocations,
  unitCount,
  showBarcodes,
  showPhotos,
  showDescription,
  showSpecialCase,
  showLabels,
  showAge,
  showTypeChip,
  onOpenNote,
  onChangeQuantity,
  onRemove,
  onClear,
  onBrowseNotes,
}: CollectionViewProps) {
  return (
    <section className={styles.section}>
      <div className={`${styles.toolbar} ${styles.noPrint}`}>
        <div>
          <h2 className={styles.heading}>Collection</h2>
          <p className={styles.subheading}>
            {unitCount === 0
              ? 'Empty'
              : `${unitCount} item${unitCount === 1 ? '' : 's'} · ${rows.length} note${
                  rows.length === 1 ? '' : 's'
                }`}
          </p>
        </div>
        <div className={styles.toolbarActions}>
          {rows.length > 0 && (
            <>
              <button
                type="button"
                className={styles.secondaryBtn}
                onClick={onClear}
              >
                <Trash2 size={16} />
                Clear
              </button>
              <button
                type="button"
                className={styles.primaryBtn}
                onClick={() => window.print()}
              >
                <Printer size={16} />
                Print / PDF
              </button>
            </>
          )}
        </div>
      </div>

      {rows.length === 0 ? (
        <div className={`${styles.empty} ${styles.noPrint}`}>
          <Layers size={28} strokeWidth={1.75} />
          <p className={styles.emptyTitle}>Collection is empty</p>
          <p className={styles.emptyText}>
            Add notes from the wall to build a collection for printing.
          </p>
          {onBrowseNotes && (
            <button
              type="button"
              className={styles.emptyCta}
              onClick={onBrowseNotes}
            >
              Browse notes
            </button>
          )}
        </div>
      ) : (
        <>
          <div className={`${styles.grid} ${styles.noPrint}`}>
            {rows.map(({ item, note }) => {
              if (!note) {
                return (
                  <div key={item.noteId} className={styles.item}>
                    <div className={styles.missingCard}>
                      <p className={styles.missing}>Missing note</p>
                      <button
                        type="button"
                        className={styles.removeBtn}
                        aria-label="Remove from collection"
                        onClick={() => onRemove(item.noteId)}
                      >
                        <X size={14} />
                      </button>
                    </div>
                  </div>
                );
              }

              return (
                <div key={item.noteId} className={styles.item}>
                  <NoteCard
                    note={note}
                    labels={labels}
                    noteTypes={noteTypes}
                    stockLocations={stockLocations}
                    selecting={false}
                    selected={false}
                    cartQuantity={0}
                    showBarcodes={showBarcodes}
                    showPhotos={showPhotos}
                    showDescription={showDescription}
                    showSpecialCase={showSpecialCase}
                    showLabels={showLabels}
                    showAge={showAge}
                    showTypeChip={showTypeChip}
                    onOpen={onOpenNote}
                    onToggleSelect={() => {}}
                    onEnterSelect={() => {}}
                    onRangeSelect={() => {}}
                  />
                  <div className={styles.itemControls}>
                    <div className={styles.qtyGroup}>
                      <button
                        type="button"
                        className={styles.qtyBtn}
                        aria-label="Decrease quantity"
                        onClick={() =>
                          onChangeQuantity(item.noteId, item.quantity - 1)
                        }
                      >
                        <Minus size={14} />
                      </button>
                      <span key={item.quantity} className={styles.qty}>
                        {item.quantity}
                      </span>
                      <button
                        type="button"
                        className={styles.qtyBtn}
                        aria-label="Increase quantity"
                        onClick={() =>
                          onChangeQuantity(item.noteId, item.quantity + 1)
                        }
                      >
                        <Plus size={14} />
                      </button>
                    </div>
                    <button
                      type="button"
                      className={styles.removeBtn}
                      aria-label="Remove from collection"
                      onClick={() => onRemove(item.noteId)}
                    >
                      <X size={14} />
                    </button>
                  </div>
                </div>
              );
            })}
          </div>

          <div className={`${styles.printSheet} ${styles.printOnly}`}>
            <header className={styles.printHeader}>
              <h1 className={styles.printTitle}>TechLib collection</h1>
              <p className={styles.printMeta}>
                {unitCount} item{unitCount === 1 ? '' : 's'} · {rows.length} note
                {rows.length === 1 ? '' : 's'} · {new Date().toLocaleDateString()}
              </p>
            </header>

            <div className={styles.tableWrap}>
              <table className={styles.table}>
                <thead>
                  <tr>
                    <th className={styles.colQty} scope="col">
                      Qty
                    </th>
                    <th className={styles.colIcon} scope="col">
                      <span className={styles.srOnly}>Type</span>
                    </th>
                    <th className={styles.colBarcode} scope="col">
                      {showBarcodes ? 'Barcode' : 'Part number'}
                    </th>
                    <th className={styles.colStatus} scope="col">
                      Guideline
                    </th>
                    <th className={styles.colType} scope="col">
                      Type
                    </th>
                    <th className={styles.colLabels} scope="col">
                      Labels
                    </th>
                    <th className={styles.colSpecial} scope="col">
                      Special
                    </th>
                  </tr>
                </thead>
                <tbody>
                  {rows.map(({ item, note }) => {
                    const type = note
                      ? noteTypeById(noteTypes, note.categoryId)
                      : null;
                    const Icon = noteTypeIcon(type?.icon);
                    const lines = note ? resolveGuidelineLines(note) : [];
                    const status =
                      lines.length === 0
                        ? null
                        : lines
                            .map((line) => {
                              const action =
                                DISPOSITIONS.find((d) => d.id === line.action)
                                  ?.short ?? line.action;
                              return `${line.when} → ${action}`;
                            })
                            .join('; ');
                    const typeName = note
                      ? categoryLabel(note.categoryId, noteTypes)
                      : null;
                    const noteLabels = labels.filter((l) =>
                      (note?.labelIds ?? []).includes(l.id),
                    );
                    const special = (note?.specialCase ?? '').trim();
                    const title = note?.title.trim() ?? '';

                    return (
                      <tr key={item.noteId}>
                        <td className={styles.colQty}>
                          <span className={styles.qtyPrint}>{item.quantity}</span>
                        </td>
                        <td className={styles.colIcon} aria-hidden>
                          <Icon size={16} strokeWidth={1.75} />
                        </td>
                        <td className={styles.colBarcode}>
                          {title ? (
                            showBarcodes ? (
                              <Barcode title={title} scannable />
                            ) : (
                              <span className={styles.titleFallback}>{title}</span>
                            )
                          ) : (
                            <span className={styles.missing}>Missing note</span>
                          )}
                        </td>
                        <td className={styles.colStatus}>{status || '—'}</td>
                        <td className={styles.colType}>{typeName || '—'}</td>
                        <td className={styles.colLabels}>
                          {noteLabels.length > 0
                            ? noteLabels.map((l) => `#${l.name}`).join(' ')
                            : '—'}
                        </td>
                        <td className={styles.colSpecial}>{special || '—'}</td>
                      </tr>
                    );
                  })}
                </tbody>
              </table>
            </div>
          </div>
        </>
      )}
    </section>
  );
}
